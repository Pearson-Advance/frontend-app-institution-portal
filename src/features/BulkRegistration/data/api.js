...
export async function uploadCSV(file) {
  ...
  try {
    ...
    const totalRows = Number(data?.errors.summary?.total_rows || 0);
    const created = Number(data?.errors.summary?.created || 0);
    const existed = Number(data?.errors.summary?.existed || 0);
    const failed = Number(data?.errors.summary?.failed || 0);
